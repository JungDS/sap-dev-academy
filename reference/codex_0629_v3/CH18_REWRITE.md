...
- 고급 constructor expression은 현재 커리큘럼 사용처가 없으므로 CH18 본문에서 확장하지 않는다.
...
- 버튼: `classic 실행`, `modern 실행`, `없는 ID 테스트`, `line_exists 제거`, `경계 검사`.
- 상태: classic result, modern result, exception risk, message text, boundary warnings.
...
CH18-L07의 목표는 최신 문법 과시가 아니다. 이미 배운 콘서트 예매 흐름을 유지하면서 선언 위치를 좁히고, 값 생성은 `VALUE`로 표현하고, 한 행 조회는 table expression과 보호 조건으로 다루며, 메시지는 string template으로 읽기 좋게 만드는 것이다.
...
10. `+=`는 inline declaration인가, calculation assignment인가?
11. CH18에서 아직 열지 않는 SQL, OO, 예외 처리 경계는 무엇인가?
