...
| 예외 | `TRY/CATCH/CLEANUP`, `RAISE EXCEPTION`, `RAISING`, `CX_STATIC_CHECK`, `CX_DYNAMIC_CHECK`, `CX_NO_CHECK`, `CX_ROOT`, `RESUME` 개념 | T100 메시지 예외 설계 심화 |
...
| 예외 | `abaptry.htm`, `abapcatch_try.htm`, `abapcleanup.htm`, `abapraise_exception.htm`, `abapresume.htm` | `TRY/CATCH/CLEANUP`, `RAISE EXCEPTION`, `RESUME` 제한 |
| 예외 계층 | `abenabap_exception_classes.htm`, `abenexception_categories.htm`, `abenexception_texts.htm` | `CX_STATIC_CHECK`, `CX_DYNAMIC_CHECK`, `CX_NO_CHECK`, `CX_ROOT`, `get_text( )` |
...
상태/피드백:
- `exceptionRaised`, `caughtBy`, `cleanupExecuted`
- 구체 catch 전에 `cx_root`를 두면 "넓은 catch가 먼저 잡아버림"이라고 표시한다.
- `RAISING` 누락 시 "method 계약에 예외 가능성이 드러나지 않음"이라고 안내한다.

### 정리

Exception은 실패를 호출자에게 명확히 전달하는 객체다. `RAISE EXCEPTION`으로 던지고, `TRY/CATCH`로 잡고, checked exception은 `RAISING` 계약을 통해 드러낸다.
...
6. 잔여석과 같은 값을 넣고 handler를 등록해 `sold_out` 이벤트가 호출되는지 확인한다.
...
- `raisedException`, `raisedEvent`, `handlerLog`
...
CH20-L10은 OO 문법을 업무 객체로 묶는 실습이다. `ZCL_BOOKING_MANAGER`는 constructor로 상태를 받고, private attribute로 보호하며, public method로 잔여석과 예매 가능 여부를 제공하고, 실패는 예외로, 사건은 이벤트로 알린다.
