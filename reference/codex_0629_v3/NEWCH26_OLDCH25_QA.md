...
> 대상 산출물: `reference/codex_0629_v3/CH25_REWRITE.md`  
> 작업 단위: CH25 1개 챕터  
...
| 산출 파일 | `CH25_REWRITE.md`, `CH25_QA.md` |
...
CH25는 Classic ABAP Lock Object와 동시성 제어 장이다. RAP ETag 구현, ALV edit event 저장 흐름, SM12 운영 정책, enqueue server 운영 심화는 후속 또는 운영 범위로 보류한다.
...
판정: 원본 5개 레슨이 모두 별도 절로 반영되었고, CH24와 이어지는 동시성 사고방식으로 확장했다.
...
확인 결과: CH25에서 사용하는 Lock Object, generated lock function module, `FOREIGN_LOCK`, `SM12`, `_SCOPE`, pessimistic/optimistic 설명은 공식 문서와 정합한다.
...
| enqueue server 운영, SM12 장애 복구 | 운영 심화 범위 |
| parallel processing lock 전략 | CH32 이후 성능/대량 처리 심화 범위 |
| multi-table lock object 고급 설계 | CH25에서는 foreign key lock 개념만 간접 언급 |

판정: CH25는 Classic ABAP Lock Object와 동시성 제어에 집중했고, 후속 장의 구현 영역으로 침범하지 않았다.
...
| 무엇인가 | Lock Object, generated FM, lock mode, `_SCOPE`, pessimistic/optimistic을 책임별로 설명 |
| 어떻게 확인하는가 | `SE11`, generated FM, 두 세션 enqueue, `SM12`, commit/rollback 후 lock 상태 확인 |
| 실수/주의 | lock object만 만들고 호출 누락, key 과다/과소, `foreign_lock` 미처리, SQL 자동 차단 오해, `_SCOPE` 단정 |
...
- Classic ABAP 공식 문서를 `C:\ABAP_DOCU_HTML`에서 직접 확인하고 `_SCOPE`, `FOREIGN_LOCK`, generated FM, `SM12`를 반영했다.
- 모든 레슨에 확인 절차와 체험형 학습 설계를 포함했다.
- R15 게이팅을 지키며 RAP ETag 구현, ALV edit event, BAPI/RFC, enqueue 운영 심화를 후속 범위로 보류했다.

판정: `CH25_REWRITE.md`는 v3 품질 기준으로 사용 가능하다.
