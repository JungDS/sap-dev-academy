...
| EML | handler 내부 `READ ENTITIES ... IN LOCAL MODE`, response `failed/reported` | 일반 프로그램에서의 복잡한 `MODIFY ENTITIES`, `COMMIT ENTITIES`, `ROLLBACK ENTITIES` 운용 |
...
| EML | `ABAPREAD_ENTITY_ENTITIES.md`, `ABAPREAD_ENTITY_ENTITIES_FIELDS.md`, `ABAPIN_LOCAL_MODE.md`, `ABAPEML_RESPONSE.md` | `READ ENTITIES`, `FIELDS ... WITH`, `IN LOCAL MODE`, `FAILED/MAPPED/REPORTED` |
...
CH23-L09는 Track-1의 종합 실습이다. CH22의 CDS 모델 위에 BDEF, Behavior Pool, Service를 얹어 예매 앱을 RAP 트랜잭션 앱으로 끌어올린다. 여기까지가 Track-1의 큰 한 바퀴다.
...
| L08 | ABAP Cloud는 released API와 Clean Core를 통해 업그레이드 안정성을 지킨다. |
| L09 | 콘서트 예매 앱을 RAP BO와 OData/Fiori 흐름으로 마무리한다. |
