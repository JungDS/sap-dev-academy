...
> 현재 적용 번호: OLDCH01~OLDCH19 -> NEWCH01~NEWCH19, `NEWCH20_OLDCH99`는 Advanced ABAP SQL 신규 장 예약, OLDCH20~OLDCH26 -> NEWCH21~NEWCH27, `NEWCH28_OLDCH99`는 Dynamic ABAP 신규 장 예약, `NEWCH29_OLDCH99`는 Advanced String Processing and Regex 신규 장 예약.
...
| CH18 | `LET`, `THROW` 포함 복잡한 expression | `LET`은 CH18에서 expression 독해 기본형으로 회수, `THROW`는 CH20 예외 처리에서 회수 | 후속 New Syntax 전용 장 없음 | 부분 회수. CH20 보강 필요 |
...
|---|---|---|---|
| CH18 `LET`, `THROW` expression | `C:\ABAP_DOCU_HTML\abaplet.htm`, `abenlet_abexa.htm`, `abenvalue_itab_let_abexa.htm`, `abenconditional_expression_result.htm`, `abenabap_exceptions.htm`, `abapraise_exception_message.htm` 확인 | **분할 보강 필요, CH18 부분 완료**. `LET`은 CH18의 constructor expression 독해 범위에서 소개했고, `THROW`는 조건식 안에서 예외를 발생시키는 문법이므로 예외 클래스 학습 전인 CH18에서 깊게 다루지 않았다 | CH18에는 `LET ... IN`을 `VALUE`/`COND`/`REDUCE` 안 보조 변수로 읽는 법, 평가 순서, 이름 충돌, 과밀 표현 주의를 반영했다. CH18 끝에 `THROW`는 예외 학습 후 회수한다고 명시했다. 남은 조치는 CH20 예외 레슨에 `COND`/`SWITCH`의 `ELSE THROW cx_...( )` 형태와 `RAISE EXCEPTION`과의 차이를 추가하는 것이다 |
...
1. CH18은 `LET` 보강을 완료했고, `THROW`는 CH20 예외 학습 후 회수한다.
...
1. CH18/CH20 사이에서 `LET`과 `THROW` expression의 소유권은 분리했다. CH18의 `LET` 보강은 완료했고, CH20의 `THROW` 보강이 남았다.
...
권장 위치는 **OLDCH26과 OLDCH27 사이**이다. 단, P1 판정으로 Advanced ABAP SQL 신규 장이 OLDCH19와 OLDCH20 사이에 먼저 들어가므로, 새 파일명 기준 Dynamic ABAP은 `NEWCH28_OLDCH99`가 된다.
