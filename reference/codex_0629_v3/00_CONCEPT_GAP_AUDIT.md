...
> 현재 적용 번호: OLDCH01~OLDCH19 -> NEWCH01~NEWCH19, `NEWCH20_OLDCH99`는 Advanced ABAP SQL 신규 장 작성 완료, OLDCH20~OLDCH26 -> NEWCH21~NEWCH27, `NEWCH28_OLDCH99`는 Dynamic ABAP 신규 장 예약, `NEWCH29_OLDCH99`는 Advanced String Processing and Regex 신규 장 예약.
...
| CH20 | `FRIENDS`, `ALIASES`, RTTI/RTTC | 기본 범위 밖 | `FRIENDS`/`ALIASES`는 CH26, RTTI/RTTC는 신규 Dynamic ABAP 장 소유 | 정밀 판정 완료. 보강 필요 |
...
| CH24 | RAP `MODIFY/COMMIT/ROLLBACK ENTITIES` | RAP/ABAP Cloud 후속 심화 | CH23에서 개념 지도 보강 완료. 외부 consumer 실행 실습은 CH36 capstone 산출물 작성 시 회수 필요 | 예정 회수 |
...
| P1 | RAP EML transaction: `MODIFY ENTITIES`, `COMMIT ENTITIES`, `ROLLBACK ENTITIES` | CH23/CH24 보류, CH36 일부 `MODIFY`만 존재 | CH23 개념 지도 보강 완료. CH36 외부 consumer 실행 실습은 후속 v3 작성 시 확인 | CH36 capstone에서 `MODIFY` + `COMMIT` + 실패 시 `ROLLBACK` 실습 회수 필요 |
...
| RAP EML transaction: `MODIFY ENTITIES`, `COMMIT ENTITIES`, `ROLLBACK ENTITIES` | `C:\ABAP_DOCU_DOWNLOAD\ABAP_DOCU\abap-docs-main\docs\standard\md\ABAPMODIFY_ENTITY_ENTITIES.md`, `ABAPCOMMIT_ENTITIES.md`, `ABAPROLLBACK_ENTITIES.md`, `ABAPEML_RESPONSE.md`, `ABAPIN_LOCAL_MODE.md`, `ABENRAP_SAVE_SEQ_GLOSRY.md` 확인 | **분담 보강 중, CH23 완료**. RAP 입문과 capstone 사이에서 반드시 회수해야 한다. 별도 신규 장보다는 CH23/CH36 분담이 적절 | `NEWCH24_OLDCH23`에는 consumer/provider, `IN LOCAL MODE`, transactional buffer, `FAILED`/`REPORTED`, `MODIFY ENTITIES` -> `COMMIT ENTITIES`/`ROLLBACK ENTITIES` 개념 지도를 반영했다. CH36 capstone에는 일반 ABAP consumer 관점의 실행 실습을 넣어야 한다. Behavior pool 내부에서 `COMMIT/ROLLBACK ENTITIES`를 호출하지 않는 경계는 CH23에 반영 완료 |
...
3. RAP EML transaction은 신규 장보다 **CH23 개념 지도 + CH36 capstone 실습**으로 회수하는 것이 맞다. CH23 개념 지도는 `NEWCH24_OLDCH23`에 반영 완료했고, CH36 실행 실습은 후속 v3 작성 시 회수한다.
...
| OO 고급 언어 요소: `FRIENDS`, `ALIASES`, RTTI/RTTC, T100 예외 텍스트 | `C:\ABAP_DOCU_HTML\abenfriends.htm`, `abapclass_local_friends.htm`, `abapaliases.htm`, `abenrtti.htm`, `abencase_type_of_rtti_abexa.htm`, `abencreate_data_via_rttc_abexa.htm`, `abenexception_texts_t100.htm`, `abenif_t100_message.htm` 확인 | **분할 보강 필요, T100은 CH20 완료**. 하나의 "고급 OO" 장으로 묶기보다 소유 개념별로 회수해야 한다. RTTI/RTTC는 P1의 Dynamic ABAP 신규 장 소유가 맞다. T100 예외 텍스트는 CH20 예외 실무성 보강으로 회수했다. `FRIENDS`/`ALIASES`는 CH26에서 읽기용 고급 OO 키워드로 최소 설명한다 | CH20에 `IF_T100_MESSAGE`, `SCX_T100KEY`, 메시지 클래스 기반 예외 텍스트, `MESSAGE oref` 감각을 반영했다. 신규 Dynamic ABAP 장에는 RTTI/RTTC를 본문 레슨으로 배치한다. CH26에는 `FRIENDS`가 캡슐화 예외 통로이고 남용 위험이 크다는 점, `ALIASES`가 interface component 이름을 짧게 노출하는 문법이라는 점을 읽기용으로 정리한다 |
...
| regex/SUBMATCHES advanced string processing | `C:\ABAP_DOCU_HTML\abapfind_pattern.htm`, `abapfind_options.htm`, `abapreplace_pattern.htm`, `abapreplace_options.htm`, `abenregex_pcre_syntax.htm`, `abenregex_system_classes.htm`, `abenstring_functions_regex.htm` 확인 | **신규 장 필요**. CH04의 `FIND`/`REPLACE` 기초로는 `PCRE`, `REGEX`, `SUBMATCHES`, `RESULTS`, `CL_ABAP_REGEX`를 회수할 수 없다. Dynamic ABAP과 성격이 달라 같은 장에 합치면 초점이 흐려진다 | 권장 위치는 P1 신규 Dynamic ABAP 장 바로 뒤, OLDCH27 ALV event 장 앞이다. Advanced SQL 삽입을 반영한 번호는 `NEWCH28_OLDCH99` Dynamic ABAP, `NEWCH29_OLDCH99` Advanced String Processing and Regex이다. 최소 레슨은 PCRE 기초, `FIND PCRE`, `MATCH COUNT/OFFSET/LENGTH`, `SUBMATCHES`, `RESULTS`, `REPLACE PCRE`, `CL_ABAP_REGEX`/`CL_ABAP_MATCHER`, 로그/이메일/코드 패턴 검증 실습이다 |
...
5. RAP EML의 `MODIFY/COMMIT/ROLLBACK ENTITIES`는 CH23 개념 지도까지 보강 완료했다. 남은 문제는 CH36 capstone에서 외부 consumer 실행 실습과 Draft/ETag 경계를 충분히 회수하는지이다.
...
| 신규 장 | 제목 후보 | 핵심 레슨 |
|---|---|---|
| NEWCH28_OLDCH99 | Dynamic ABAP: Field Symbol 심화와 Generic Programming | L01 typed vs generic Field Symbol, L02 `TYPE any`/`ANY TABLE`/generic formal parameter, L03 `ASSIGN`/`UNASSIGN`/`IS ASSIGNED`, L04 `ASSIGN COMPONENT`, L05 `ASSIGN (name)`과 보안/덤프 위험, L06 `REF TO data`와 `CREATE DATA`, L07 RTTS/RTTI로 구조와 타입 읽기, L08 실습: 동적 구조 검사기 |

regex/SUBMATCHES는 P2 정밀 판정 결과 별도 신규 장을 권장한다. 두 가지 선택지를 검토했으나, 최종 권고는 독립 장이다.

| 선택지 | 판단 |
|---|---|
| NEWCH29_OLDCH99 `Advanced String Processing and Regex` | 권장. `FIND PCRE`, `REPLACE PCRE`, `MATCH COUNT/OFFSET/LENGTH`, `SUBMATCHES`, `RESULTS`, `CL_ABAP_REGEX`, 로그/이메일/코드 패턴 검증을 체계적으로 다룰 수 있다. |
