...
| 정밀 판정 대상 | 후속 소유 챕터가 불명확하거나, 유일한 소유 챕터에서 빠진 가능성이 있음 | 2차 감사 또는 해당 챕터 재작업 후보 |
...
| CH08 | 성능 심화 | CH32 | `NEWCH35_OLDCH32`에서 ST05, SAT, SQLM/SWLT, SELECT-in-LOOP 제거, 대량 처리 설계로 회수 완료 | 정상 회수 |
...
| CH09 | 직접 F4 구현: dynpro `PROCESS ON VALUE-REQUEST` | CH16 | `NEWCH16_OLDCH16` L05/L10에서 `PROCESS ON VALUE-REQUEST`, `FIELD ... MODULE`, `F4IF_FIELD_VALUE_REQUEST`, `F4IF_INT_TABLE_VALUE_REQUEST`, `DYNP_VALUES_READ/UPDATE`로 회수 완료 | 정상 회수 |
| CH09 | DB 변경/Transaction/MESSAGE/JOIN/성능 | CH24/CH15/CH13/CH32 | DB 변경/Transaction은 CH24, MESSAGE는 CH15, JOIN은 CH13, 성능은 `NEWCH35_OLDCH32`에서 회수 완료 | 정상 회수 |
...
| CH14 | Search Help exit, 동적 F4 제어 | CH15/CH16 | selection screen F4는 CH15에서 회수했고, dynpro F4 우선순위와 Search Help exit 경계는 `NEWCH16_OLDCH16` L05에서 회수 완료 | 정상 회수 |
...
| CH16 | `TABLES dbtab` 기반 Dynpro 화면-프로그램 데이터 교환 | Screen Programming 필수 classic 키워드 | `NEWCH16_OLDCH16` L04/L10에서 Dictionary 기반 dynpro field, `TABLES zconcert` interface work area, PBO/PAI data transport, CH12 `TABLES`와의 차이를 회수 완료 | 정상 회수 |
...
| CH19 | Native SQL/ADBC | CH33 | `NEWCH36_OLDCH33` L03/L05에서 ADBC/Native SQL, `CL_SQL_STATEMENT`, placeholder/binding, client 처리, `CX_SQL_EXCEPTION`, `EXEC SQL` 경계로 회수 완료 | 정상 회수 |
...
| CH21 | 실제 저장/취소, CDS/RAP, 성능 | CH24, CH22/23, CH32 | 실제 저장/취소는 CH24, CDS/RAP은 CH22/23, 성능은 `NEWCH35_OLDCH32`에서 회수 완료 | 정상 회수 |
...
| CH24 | 병렬 처리/성능 튜닝 | CH32 | `NEWCH35_OLDCH32` L05에서 package, restart, pushdown 판단, 병렬 가능성, lock/resource 위험으로 회수 완료 | 정상 회수 |
| CH25 | ALV edit event, BAPI/RFC, 병렬 처리 | CH28, CH30, CH32 | ALV edit event는 `NEWCH31_OLDCH28`에서 회수 완료. BAPI/RFC는 `NEWCH33_OLDCH30`에서 회수 완료. 병렬 처리/성능은 `NEWCH35_OLDCH32`에서 회수 완료 | 정상 회수 |
...
| CH25 | enqueue server 운영, SM12 장애 복구 | CH25/운영 경계 | `NEWCH26_OLDCH25` L07에서 SM12 lock entry 판독, stuck lock 원인 분류, 수동 삭제 위험, Basis 에스컬레이션 경계를 회수 완료. enqueue server HA/구성 운영은 Basis 영역으로 제외 | 정상 회수 |
| CH25 | multi-table lock object 고급 설계 | CH25 | `NEWCH26_OLDCH25` L06에서 header/item 업무 root key, foreign key dependency, lock granularity, 과잠금/분리잠금 주의를 회수 완료 | 정상 회수 |
...
아래 항목은 1차 감사에서 "CH26의 `COND` 사례와 같은 구조적 갭이 될 수 있음"으로 분류했던 후보들이다. 각 후보는 section 4.1~4.3에서 공식 문서와 전체 커리큘럼 목표를 대조해 판정했고, 현재 처리 결과를 표에 반영했다.

| 우선순위 | 후보 | 감지 위치 | 감사 당시 상태 | 처리 결과 |
|---|---|---|---|---|
| P1 | Dynpro 직접 F4 `PROCESS ON VALUE-REQUEST` | CH09가 CH16으로 넘김 | `NEWCH16_OLDCH16` L05/L10에서 보강 완료 | 재작업 완료. CH16에서 PAI/PBO와 별도로 POV/POH, 직접 F4, 화면값 읽기/반영 경계를 회수 |
...
| P3 | Search Help exit / dynpro 동적 F4 제어 | CH14 보류, CH15 selection-screen F4 회수 | `NEWCH16_OLDCH16` L05에서 dynpro 측 회수 완료 | 재작업 완료. 직접 구현은 POV로 다루고, Search Help exit는 DDIC Search Help 확장 경계로 제한 |
...
| P1 | `TABLES dbtab` for Dynpro Dictionary screen fields | CH16 Screen Programming | `NEWCH16_OLDCH16` L04/L10에서 보강 완료 | 재작업 완료. Screen Painter DDIC field와 `TABLES` work area 연결, PBO/PAI transport, selection screen `TABLES`와의 차이를 반영 |
...
> 시작일: 2026-07-03 KST
> 범위: section 4의 P1 후보 판정 기록. P2/P3 결과는 section 4.2/4.3에 별도로 정리한다.

P1은 단순 "나중에 언급하면 좋음"이 아니라, 공식 문서상 독립 키워드와 실행 모델이 있고 현재 커리큘럼 안에서 소유 위치가 없거나 후속 장에서 빠진 항목이다. 따라서 P1 판정은 "본문 보강", "신규 장 작성", "명시 제외" 중 하나로 확정했고, 필요한 보강은 현재 v3 산출물에 반영 완료했다.

| P1 후보 | 공식문서 수동 확인 | 정밀 판정 | 필요한 조치 |
|---|---|---|---|
| Dynpro 직접 F4 `PROCESS ON VALUE-REQUEST` | `C:\ABAP_DOCU_HTML\abenabap_dynpros_value_help.htm`, `abenabap_dynpros_value_help_mod.htm`, `abenabap_dynpros_field_help.htm`, `abenabap_dynpro_call_dialog_mod.htm` 확인 | **보강 완료**. CH09/CH14가 CH16으로 넘긴 것이 맞고, CH16이 Screen Programming 장이므로 `NEWCH16_OLDCH16`에서 회수했다. 신규 장은 불필요 | L05/L10에 `PROCESS ON VALUE-REQUEST`, `PROCESS ON HELP-REQUEST`, `FIELD field MODULE mod`, Dictionary F4와 직접 F4의 우선순위, POV 시점의 화면값 읽기/반영 주의(`DYNP_VALUES_READ`/`DYNP_VALUES_UPDATE` 또는 F4 FM 호출 설계)를 반영했다 |
...
| `TABLES dbtab` for Dynpro Dictionary screen fields | `C:\ABAP_DOCU_HTML\abaptables.htm`, `abenabap_dynpros_fields.htm`, `abenabap_dynpros_processing.htm` 확인 | **보강 완료**. CH12의 `SELECT-OPTIONS ... FOR`용 `TABLES`와 다른 목적이므로 `NEWCH16_OLDCH16` L04/L10에서 Screen Painter Dictionary field 연결로 회수했다 | L04/L10에 `TABLES dbtab`이 Dictionary structure와 같은 모양의 table work area를 선언한다는 점, 화면 field 이름과 program data object 연결, PBO/PAI 전송 감각, class 안에서는 쓰지 않는 classic 경계를 반영했다 |

P1 정밀 판정의 결론은 다음과 같다.

1. CH16은 **두 가지 보강**을 완료했다: Dynpro POV/POH와 Dynpro용 `TABLES dbtab`.
...
|---|---|---|---|
| Search Help exit / dynpro 동적 F4 제어 | `C:\ABAP_DOCU_HTML\abenabap_dynpros_value_help.htm`, `abenabap_dynpros_value_help_mod.htm`, `abenabap_dynpros_value_help_dynp.htm`, `abendynpro_f4_help_dic_abexa.htm`, `C:\ABAP_DOCU_DOWNLOAD\ABAP_DOCU\abap-cheat-sheets-main\18_Dynpro.md` 확인 | **CH16 보강에 흡수 완료**. dynpro 동적 F4는 P1의 `PROCESS ON VALUE-REQUEST`와 같은 보강 묶음이다. Search Help exit는 DDIC Search Help를 확장하는 고급 기법이므로 CH16에서 구현까지 다루지는 않는다 | `NEWCH16_OLDCH16` L05에 "F4 우선순위: DDIC Search Help -> dynpro field binding -> Search Help exit 고려 -> POV 직접 구현" 흐름을 반영했다. 본문 실습은 `PROCESS ON VALUE-REQUEST`, `F4IF_INT_TABLE_VALUE_REQUEST`, `DYNP_VALUES_READ/UPDATE`까지만 작성했다. Search Help exit는 "DDIC Search Help 자체를 강화하는 고급 확장"으로 경계만 둔다 |
...
4. CH16 보강은 P1의 `PROCESS ON VALUE-REQUEST`/`TABLES dbtab`와 P3의 Search Help exit 경계를 한 번에 반영 완료했다.
...
| BAPI/RFC, Application Log, AMDP/ADBC, Forms/Output | BAPI/RFC는 `NEWCH33_OLDCH30`, Application Log는 `NEWCH38_OLDCH35`, AMDP/ADBC는 `NEWCH36_OLDCH33`, Forms/Output은 `NEWCH37_OLDCH34`에서 작성 완료했다. |
...
다만 CH26의 `COND` 사례와 같은 구조적 갭 가능성이 확인된 항목이 있었고, 현재 v3 산출물 기준으로 다음과 같이 회수했다.
...
2. CH16 Screen Programming에서 `TABLES dbtab` 기반 Dictionary screen field와 program data object 연결 설명은 `NEWCH16_OLDCH16` L04/L10에서 회수 완료했다.
3. CH09/CH14에서 후속으로 넘긴 Dynpro value-request는 `NEWCH16_OLDCH16` L05/L10에서 회수 완료했다.
...
1. CH16은 P1 보강 때 Search Help exit와 직접 POV 구현의 경계를 함께 설명하도록 `NEWCH16_OLDCH16` L05에 반영 완료했다.
...
현재 감사 기준 권고 중 CH16 Screen Programming 보강은 `NEWCH16_OLDCH16`에서 완료했고, CH25 Lock Object 보강은 `NEWCH26_OLDCH25` L06/L07에서 완료했다. **동적/제네릭 ABAP 신규 장은 `NEWCH28_OLDCH99`로 작성 완료**했고, regex/SUBMATCHES는 **`NEWCH29_OLDCH99` 별도 문자열 심화 장으로 작성 완료**했다. CH27 ALV 고급 Event 장은 `NEWCH30_OLDCH27`로 작성하여 ALV double click/hotspot/toolbar/user command 회수 항목을 반영했다. CH28 Editable Grid ALV 장은 `NEWCH31_OLDCH28`로 작성하여 `data_changed`, `data_changed_finished`, Cell Style, 입력 오류 protocol, `check_changed_data`, 저장 전 DML/LUW 회수 항목을 반영했다. CH29 Enhancement / BAdI / User Exit 장은 `NEWCH32_OLDCH29`로 작성하여 CH05에서 보류된 표준 확장 심화와 Clean Core 확장 판단을 회수했다. CH30 인터페이스 실무 장은 `NEWCH33_OLDCH30`으로 작성하여 BAPI/BAPIRET2, BAPI commit/rollback, RFC destination/예외, BDC/Excel/File interface, 로그/재처리/멱등성 회수 항목을 반영했다. CH31 IDoc / ALE / Gateway 장은 `NEWCH34_OLDCH31`로 작성하여 IDoc Control/Data/Status, ALE Distribution Model, IDoc status/BD87 재처리, Classic Gateway SEGW 구조, `GET_ENTITYSET`/OData query option, Gateway 운영 회수 항목을 반영했다. CH32 성능 분석과 튜닝 장은 `NEWCH35_OLDCH32`로 작성하여 ST05, SAT, SQLM/SWLT, SELECT-in-LOOP 제거, FAE/JOIN/key access, 대량 package, pushdown, 병렬 처리 판단 회수 항목을 반영했다. CH33 AMDP / ADBC / Pushdown 장은 `NEWCH36_OLDCH33`으로 작성하여 Code-to-Data 판단, AMDP procedure, ADBC/Native SQL, Open SQL/CDS/AMDP/ADBC 비교, CDS Table Function/AMDP Function, client/security/Clean Core 경계를 반영했다. CH34 Forms/Output/PDF 장은 `NEWCH37_OLDCH34`로 작성하여 BRF+ Output Management 경계를 반영했고, CH35 운영 품질 장은 `NEWCH38_OLDCH35`로 작성하여 ABAP Unit, Test Double, Background Job, Application Log 회수 항목을 반영했다.

이 문서는 1차 후보 수집 결과이며, P1은 section 4.1, P2는 section 4.2, P3는 section 4.3에서 정밀 판정을 확정했다. 현재 기록된 P1/P2/P3 보강 대상은 각 산출물에 반영 완료했다.
...
| 2026-07-06 | `NEWCH27_OLDCH26_REWRITE.md`, `NEWCH27_OLDCH26_QA.md` | CH26 고급 OO 키워드 보강 완료. P2에서 회수 대상으로 남았던 `FRIENDS`와 `ALIASES`를 L03A 보강 레슨으로 추가하고, friendship의 단방향 권한/캡슐화 위험, interface component alias의 출처 확인법, OO Access Boundary Inspector 실습을 반영했다. |
...
| 2026-07-07 | `NEWCH35_OLDCH32_REWRITE.md`, `NEWCH35_OLDCH32_QA.md` | CH32 성능 분석과 튜닝 장 작성 완료. `ST05`, SQL Trace, Identical/Redundant Selects, `SAT`, Hit List, ABAP/DB/External time, `SQLM`, `SWLT`, `FOR ALL ENTRIES`, `READ TABLE`, `BINARY SEARCH`, JOIN, SELECT-in-LOOP 제거, `GROUP BY`, `SUM`, package/restart, `COMMIT WORK`, 병렬 처리/aRFC 판단, pushdown 경계를 반영했다. |
| 2026-07-07 | `NEWCH36_OLDCH33_REWRITE.md`, `NEWCH36_OLDCH33_QA.md` | CH33 AMDP / ADBC / Pushdown 장 작성 완료. Code-to-Data 판단, Open SQL/CDS 우선순위, `IF_AMDP_MARKER_HDB`, `AMDP OPTIONS READ-ONLY`, `BY DATABASE PROCEDURE`, `USING`, ADBC `CL_SQL_STATEMENT`/`CL_SQL_PREPARED_STATEMENT`/`CL_SQL_RESULT_SET`, placeholder/binding, `CX_SQL_EXCEPTION`, Native SQL client/security 경계, `BY DATABASE FUNCTION`, `FOR TABLE FUNCTION`, `DEFINE TABLE FUNCTION`, CDS Table Function/AMDP Function 경계를 반영했다. |
| 2026-07-08 | `00_CONCEPT_GAP_AUDIT.md` | CH16 stale 정밀 판정 행 정리 완료. `NEWCH16_OLDCH16` L04/L05/L10에 이미 반영된 `TABLES dbtab`, `PROCESS ON VALUE-REQUEST`, `PROCESS ON HELP-REQUEST`, Search Help exit 경계, `DYNP_VALUES_READ/UPDATE` 회수 상태를 감사표에 반영했다. |
| 2026-07-08 | `NEWCH26_OLDCH25_REWRITE.md`, `NEWCH26_OLDCH25_QA.md`, `00_CONCEPT_GAP_AUDIT.md` | CH25 잔여 정밀 판정 항목 보강 완료. L06에 header/item 복합 Lock Object, 업무 root key, foreign key dependency, lock granularity를 추가했고, L07에 SM12 lock entry 판독, stuck lock 원인 분류, 수동 삭제 위험, Basis 에스컬레이션 경계를 추가했다. |
| 2026-07-08 | `00_CONCEPT_GAP_AUDIT.md`, `NEWCH37_OLDCH34_QA.md` | 정밀 판정 문서 표현 정리 완료. section 4를 "후보 및 처리 결과" 기록으로 바꾸고, BRF+ QA의 과거 상태 표현을 "정밀 판정 후보"로 정리했다. |
