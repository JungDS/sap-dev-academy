// ===== association-path-simulator 엔진 JS — CDS Association 경로 시뮬레이터 (CH23-L03) =====
// 공연 선택→_Perf 경로로 회차 따라가기. 토글: 노출(off→소비자 경로 못 봄)·concert_id 유지(제거→ON source 사라짐)
// ·cardinality([1..1]인데 회차 여러개→불일치 — 교육용 시뮬 경고, 실제 cardinality는 서술적)·JOIN 보기(CH13 INNER JOIN 비교).
// 코드패널 select list = 본문 L03 코드와 동일 5요소(key concert_id·artist·venue·capacity·_Perf). 데이터=window.APS_CFG.
...
(function(){
  ...
  function evalMsg(){
    ...
    if(st.card==='1..1' && n>1){ setMsg('cardinality <b>[1..1]</b>로 선언했지만 이 공연에는 회차가 <b>'+n+'개</b>입니다 — 모델과 데이터 현실이 어긋납니다. 한 공연에 회차가 여럿이면 <b>[0..*]</b>가 맞습니다. <small>(이 경고는 교육용 시뮬레이션 — 실제 cardinality는 데이터를 강제하지 않는 <b>서술</b>이라, 어긋나도 대개 구문 경고·결과 이상으로 나타날 뿐입니다.)</small>','warn'); return; }
    setMsg('<b>'+esc(st.sel)+'</b>의 <code>concert_id</code>로 <b>_Perf</b> 경로를 따라가 회차 <b>'+n+'건</b>을 얻었습니다 (['+st.card+']). 경로(path)를 쓰면 내부적으로 JOIN으로 변환될 수 있어요.','ok');
  }
