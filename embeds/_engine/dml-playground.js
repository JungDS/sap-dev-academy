/* dml-playground — DML 4종을 ZBOOKING 미니 테이블에 실행해 보는 체험 (CH24-L01).
   INSERT(중복키=덤프)·UPDATE(WHERE 생략=전체)·MODIFY(upsert)·DELETE(WHERE 생략=전체삭제).
...
(function () {
  ...
  function run() {
    ...
    if (op === 'INSERT') {
      if (find(v.id) >= 0) return setMsg('bad', '✕ <b>INSERT 중복키</b> — booking_id <code>' + v.id + '</code>가 이미 있습니다 → 런타임 <b>덤프</b>! 있으면 수정하려면 <b>MODIFY</b>(upsert).');
    ...
    if (op === 'MODIFY') {
      ...
      rows.push({ id: v.id, pax: v.pax || '신규승객', seats: v.seats || '1', st: v.st, by: UNAME, on: TODAY });
      render({ ids: [v.id], cls: 'ins' });
      return setMsg('ok', '✓ <b>MODIFY</b> — 키가 <b>없어</b> 새 행을 <b>추가</b>(upsert)했습니다. INSERT와 달리 중복키여도 덤프 안 남.');
    }
    if (op === 'UPDATE') {
