/* screen-modify-panel 엔진 — AT SELECTION-SCREEN OUTPUT에서 LOOP AT SCREEN으로 필드 속성을 바꾸고
   MODIFY SCREEN으로 화면에 반영하는 흐름. p_mode를 바꿔도 '화면 그리기(OUTPUT 실행)' 전에는 미반영.
   골격 계약: .smp-mode · [data-draw] · #smpPreview · #smpAttr · #smpStatus.
   config: window.SMP_CFG = { secretField } (기본 P_SECRET, mode 'A'면 보임). 높이: _autoheight.js. */
(function () {
  var CFG = window.SMP_CFG || { secretField: 'P_SECRET' };
  var pMode = '';          // '' 기본 | 'A' 고급
  var drawn = null;        // P_SECRET active 반영값(null=아직 안 그림)
  ...
  function renderPreview() {
    ...
    previewEl.innerHTML =
      '<div class="smp-srow"><span class="nm">P_MODE</span><input type="text" value="' + esc(pMode) + '" readonly></div>' +
      '<div class="smp-srow' + (secretGone ? ' gone' : '') + '"><span class="nm">P_SECRET</span><input type="text" placeholder="(고급 전용)" readonly></div>' +
      (stale ? '<div class="smp-stale">⚠ p_mode를 바꿨습니다 — ▶ 화면 그리기를 눌러야 반영됩니다(MODIFY SCREEN).</div>'
  ...
  function renderAttr() {
    ...
    attrEl.innerHTML = '<thead><tr><th>name</th><th>active</th><th>의미</th></tr></thead><tbody>' +
      '<tr><td>P_MODE</td><td class="on">1</td><td>항상 표시</td></tr>' +
      '<tr><td>P_SECRET</td><td class="' + cls + '">' + act + '</td><td>' + (drawn === 1 ? '표시' : (drawn === 0 ? '숨김(active 0)' : '미반영')) + '</td></tr></tbody>';
  }
  ...
  modeEl.addEventListener('click', function (e) {
    ...
    setStatus('', 'p_mode = <b>' + (pMode || "''") + '</b>로 바꿨습니다. 변수만 바뀌었을 뿐 — <b>화면 그리기</b>를 눌러 OUTPUT을 실행해야 화면이 바뀝니다.');
  });
  document.querySelector('[data-draw]').addEventListener('click', function () {
    drawn = (pMode === 'A') ? 1 : 0; stale = false; render();
    if (drawn === 1) setStatus('ok', '✅ OUTPUT 실행 — p_mode=A라 <code>P_SECRET</code>가 그대로 보입니다(active=1).');
    else setStatus('ok', '✅ OUTPUT 실행 — <code>LOOP AT SCREEN</code>이 <code>P_SECRET</code>의 <code>active=0</code>으로 바꾸고 <code>MODIFY SCREEN</code>으로 반영 → 필드가 숨겨졌습니다.');
  });

  render();
  setStatus('', 'p_mode를 고르고 <b>▶ 화면 그리기</b>를 눌러, OUTPUT이 화면 속성을 어떻게 바꾸는지 보세요.');
})();
