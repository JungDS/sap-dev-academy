...
// [LOOP 돌려 보기] 순회 출력(sy-tabix 대비)·[READ INDEX 시도] 판정 카드(구문 오류 포함)를 관찰.
...
(function () {
  ...
  var ki = 0, looped = false, readTried = false;
  ...
  function loopHTML() {
    ...
    var lines = k.loop.map(function (l) {
      return '<div class="ktl-out__ln"><span class="tb' + (String(l.tabix) === '0' ? ' zero' : '') + '">sy-tabix ' + l.tabix + '</span>' + esc(l.label) + '</div>';
    }).join('');
    return '<div class="ktl-out">' + lines + '<div class="ktl-out__cap">' + k.loopCap + '</div></div>';
  }
  ...
  root.addEventListener('click', function (e) {
    ...
    if (t) { ki = +t.dataset.k; looped = false; readTried = false; render(); return; }
