/* window-partition-sim 엔진 — GROUP BY(행 접힘) vs Window PARTITION BY(행 유지)를 나란히 비교하고
   ...
   config: window.WPS_CFG = { rows:[{id,part,seats}], excluded?:[{id,part,seats,status}], modes:[{key,btn,sql,desc,tone}] }.
   excluded = WHERE(예: status <> 'C')로 걸러진 원본 행 — 도식 왼쪽에 취소선으로 표시(집계에는 미포함).
...
(function () {
  ...
  function buildTable(key) {
    ...
    if (key === 'group') {
      ...
      cols = ['booking_id', 'concert_id', 'seats', 'seat_rank', 'seat_rank_dense'];
      trs = rows.map(function (r) {
  ...
  function buildShape(key) {
    var leftPills = rows.map(function (r) { return pill(r.id + ' · ' + r.seats, gcls(r.part)); }).join('') +
      excluded.map(function (r) { return pill(r.id + ' · ' + r.seats + (r.status ? ' (' + r.status + ')' : ''), 'cut'); }).join('');
    ...
    var leftLabel = excluded.length
      ? '원본 ' + (rows.length + excluded.length) + '행 · WHERE로 ' + excluded.length + '행 제외'
      : '원본 상세 ' + rows.length + '행';
    elShape.innerHTML =
      '<div class="wps-col"><span class="wps-cl">' + esc(leftLabel) + '</span><div class="wps-pills">' + leftPills + '</div></div>' +
