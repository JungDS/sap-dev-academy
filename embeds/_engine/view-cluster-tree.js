/* view-cluster-tree 엔진 — View Cluster의 계층 유지보수: 상위(공연장)를 고르면 하위(좌석등급)가 그 key로 좁혀지고,
   ...
   config: window.VCT_CFG = { concerts:[{id,name,perfs:[{no,date}]}], presets:[{no,date}] }. 높이: _autoheight.js. */
(function () {
  ...
  function hall(id) { return CFG.concerts.filter(function (h) { return h.id === id; })[0]; }
  ...
  function renderTree() {
    treeEl.innerHTML = CFG.concerts.map(function (h) {
      return '<div class="vct-node' + (sel === h.id ? ' sel' : '') + '" data-id="' + h.id + '">' +
        ...
        '<span class="cnt">회차 ' + h.perfs.length + '</span></div>' +
        '<div class="vct-child-hint">└ 회차 (concert_id=' + esc(h.id) + ')</div>';
    }).join('');
  }
  ...
  treeEl.addEventListener('click', function (e) {
    ...
    setStatus('ok', '현재 선택: <b>' + esc(sel) + ' ' + esc(hall(sel).name) + '</b> → 하위 회차는 자동으로 <b>concert_id = ' + esc(sel) + '</b> 조건으로 좁혀집니다.');
  });
  gridEl.addEventListener('click', function (e) {
    ...
    hall(sel).perfs.push({ no: p.no, date: p.date });
    renderTree(); renderPanel();
    setStatus('ok', '✅ <b>' + esc(sel) + '</b> 아래에 회차 <b>' + esc(p.no) + '</b>를 추가했습니다(concert_id가 자동으로 채워짐). View Cluster가 부모 맥락을 이어 줍니다.');
  });
