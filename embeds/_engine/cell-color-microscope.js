/* cell-color-microscope 엔진 — 셀 단위 색은 행에 LVC_T_SCOL(cellcolors)을 품는 deep structure로 만든다.
   ...
   fname을 틀리면 색 정보는 있어도 적용되지 않는다. 색 대상 셀 = colorCol(SEATS_LEFT = capacity − booked, 매진 행이면 0).
   골격 계약: .ccm-act(버튼) · #ccmTable · #ccmScol · #ccmStatus.
   config: window.CCM_CFG = { rows, colorCol, badName }. 높이: _autoheight.js. */
(function () {
  var CFG = window.CCM_CFG || { rows: [], colorCol: 'SEATS_LEFT', badName: 'SEATSLEFT' };
  ...
  function renderTable() {
    var head = '<tr><th>CONCERT_ID</th><th>PERF_NO</th><th>CAPACITY</th><th>BOOKED</th><th>SEATS_LEFT</th></tr>';
    var body = CFG.rows.map(function (r) {
      var full = isFull(r);
      // 색 계산은 됐지만 아직 연결 전이면 armed(테두리만), 연결되면 painted(칠) — 대상은 잔여석(SEATS_LEFT) 셀
      ...
      return '<tr class="' + (full ? 'full' : '') + '"><td>' + h(r.concert) + '</td><td>' + h(r.perf) + '</td>' +
        '<td>' + r.capacity + '</td><td>' + r.booked + '</td><td class="' + seatCls + '">' + (r.capacity - r.booked) + '</td></tr>';
    }).join('');
