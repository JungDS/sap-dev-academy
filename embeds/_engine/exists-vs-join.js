/* exists-vs-join 엔진 — 같은 데이터(zconcert·zbooking)에 INNER JOIN·EXISTS·IN subquery를 각각 적용해
   '행 중복'과 '존재/포함 판정' 차이를 보여 준다. JOIN은 zbooking 행을 붙여 공연이 예약 건수만큼 늘고(중복),
   EXISTS는 '예약이 하나라도 있나'만, IN은 'seats >= 3 예약이 있는 공연 ID 목록에 드는가'를 확인해 공연을 한 번씩만 돌려준다.
   ...
   config: window.EVJ_CFG = { concerts:[{id,artist}], bookings:[{b,c,seats,status}], inMinSeats?:3 }. 높이: 자체 post().
   안내문 속 공연명(취소만 있는 공연 등)도 전부 cfg에서 계산 — 엔진에 레슨 데이터 하드코딩 없음. */
(function () {
  var CFG = window.EVJ_CFG || { concerts: [], bookings: [] };
  var concerts = CFG.concerts || [];
  var bookings = CFG.bookings || [];

  var mode = 'join';        // 'join' | 'exists' | 'in'
  var removeCancel = false; // 토글: true면 status<>'C' 필터 제거
  var IN_MIN = CFG.inMinSeats || 3; // IN subquery의 seats 기준
  ...
  function titleOf(id) {
    for (var i = 0; i < concerts.length; i++) if (concerts[i].id === id) return concerts[i].artist;
    return id;
  }
  ...
  // 취소 예약만 있는 공연(토글 효과 안내문에 사용 — cfg 주도)
  function cancelOnlyNames() {
    return concerts.filter(function (c) {
      var mine = bookings.filter(function (b) { return b.c === c.id; });
      return mine.length > 0 && mine.every(function (b) { return b.status === 'C'; });
    }).map(function (c) { return c.artist; });
  }

  /* ---------- 원본 두 표(정적, cfg 주도) ---------- */
  function renderSrc() {
    var ch = '<table class="evj-t"><caption>zconcert (공연)</caption><thead><tr><th>concert_id</th><th>artist</th></tr></thead><tbody>' +
      concerts.map(function (c) {
        return '<tr><td class="mono">' + esc(c.id) + '</td><td>' + esc(c.artist) + '</td></tr>';
      }).join('') + '</tbody></table>';
    var bh = '<table class="evj-t"><caption>zbooking (예약)</caption><thead><tr><th>booking_id</th><th>concert_id</th><th>seats</th><th>status</th></tr></thead><tbody>' +
      bookings.map(function (b) {
        var cancelled = b.status === 'C';
        return '<tr class="' + (cancelled ? 'cx' : '') + '"><td class="mono">' + esc(b.b) + '</td><td class="mono">' + esc(b.c) +
          '</td><td class="mono">' + esc(b.seats == null ? '' : b.seats) +
  ...
  function renderControls() {
    modesEl.innerHTML =
      ...
      '<button type="button" data-mode="in" aria-pressed="' + (mode === 'in') + '">IN 으로 조회</button>';
    toggleEl.innerHTML =
  ...
  function renderQuery() {
    ...
    var h;
    if (mode === 'join') {
      h = 'SELECT c~concert_id, c~artist, b~booking_id\n' +
        '  FROM zconcert AS c\n' +
      ...
      h = 'SELECT c~concert_id, c~artist\n' +
        '  FROM zconcert AS c\n' +
        ' WHERE EXISTS ( SELECT concert_id FROM zbooking AS b\n' +
        '                 WHERE b~concert_id = c~concert_id' + (filter ? '\n<em>                   AND b~status &lt;&gt; \'C\'</em>' : '') + ' )\n' +
        '  INTO TABLE @gt_result.';
    } else {
      h = 'SELECT c~concert_id, c~artist\n' +
        '  FROM zconcert AS c\n' +
        ...
        '                           FROM zbooking\n' +
        '                          WHERE seats >= ' + IN_MIN + (filter ? '\n<em>                            AND status &lt;&gt; \'C\'</em>' : '') + ' )\n' +
        '  INTO TABLE @gt_result.';
    }
    queryEl.innerHTML = '<span class="lbl">' + (mode === 'join' ? 'INNER JOIN' : mode === 'exists' ? 'EXISTS 서브쿼리' : 'IN 서브쿼리') + '</span>' + h;
  }
  ...
  function joinRows() {
    ...
    concerts.forEach(function (c) {
      liveBookings(c.id).forEach(function (b) {
        rows.push({ id: c.id, title: c.artist, booking: b.b, status: b.status });
      });
    });
    return rows;
  }
  function existsRows() {
    ...
    }).map(function (c) { return { id: c.id, title: c.artist }; });
  }
  function inRows() {
    return concerts.filter(function (c) {
      return liveBookings(c.id).some(function (b) { return (b.seats || 0) >= IN_MIN; });
    }).map(function (c) { return { id: c.id, title: c.artist }; });
  }
  ...
  function renderResult() {
    ...
    if (mode === 'join') {
      ...
      resultEl.innerHTML = '<table class="evj-t res"><caption>gt_result — JOIN 결과</caption>' +
        '<thead><tr><th>concert_id</th><th>artist</th><th>booking_id</th></tr></thead><tbody>' + body + '</tbody></table>';
      renderBadges(keys.length, rows.length, dupCount);
      renderFb('join', keys.length, rows.length, dupCount);
    } else {
      rows = mode === 'exists' ? existsRows() : inRows();
      ...
      resultEl.innerHTML = '<table class="evj-t res"><caption>gt_result — ' + (mode === 'exists' ? 'EXISTS' : 'IN') + ' 결과</caption>' +
        '<thead><tr><th>concert_id</th><th>artist</th></tr></thead><tbody>' + body2 + '</tbody></table>';
      renderBadges(rows.length, rows.length, 0);
      renderFb(mode, rows.length, rows.length, 0);
    }
  }
  ...
  function renderFb(kind, outer, total, dup) {
    ...
    if (kind === 'join') {
      ...
      h = '<div class="fb good"><b>IN은 "목록에 포함되는가?"를 묻습니다.</b> 안쪽 SELECT가 먼저 ' +
        '"<b>' + IN_MIN + '석 이상</b> 예약이 있는 공연 ID 목록"을 만들고, 그 목록에 드는 공연만 남겨요. ' +
        'EXISTS처럼 행을 붙이지 않아 <b>중복이 0</b>이고, 지금 통과한 공연은 ' + outer + '개.</div>';
    }
    // 취소 토글의 효과를 한 줄 덧붙임(취소만 있는 공연명은 cfg에서 계산)
    var co = cancelOnlyNames();
    if (co.length) {
      var coNames = co.map(esc).join('·');
      h += '<div class="fb-sub">' + (removeCancel
        ? '취소 조건을 <b>뺐기</b> 때문에, 취소(C)만 있는 <b>' + coNames + '</b>도 "예약이 있는" 공연으로 잡힙니다.'
        : '취소 조건(<code>status &lt;&gt; \'C\'</code>)이 살아 있어서, 취소(C)만 있는 <b>' + coNames + '</b>는 결과에서 빠집니다.') + '</div>';
    }
    fbEl.innerHTML = h;
  }
