...
(function(){
  ...
  var STEPS=[
    ...
    {id:'zcConcert',label:'ZC_Concert (projection)'},
  ...
  function renderArts(){
    var arts=[
      ...
      {nm:'ZC_Concert', role:'소비 projection', on:st.zcConcert},
  ...
  function renderOut(){
    ...
    if(st.preview){
      ...
      html+='<div class="cbs-prev">'+
        ...
        '</div><span class="cbs-eq">두 뷰 모두 '+CONCERTS.length+'행 — projection은 행 수를 바꾸지 않습니다</span>';
    }
    if(st.consume){
  ...
  function doStep(id){
    if(id==='ziPerf'){ st.ziPerf=true; setMsg('<b>ZI_Perf 활성화.</b> 회차 기반 뷰가 준비됐습니다 — association target으로 쓸 수 있어요.','ok'); }
      ...
      if(!st.ziConcert){ setMsg('<b>활성화 실패.</b> projection 기반인 <b>ZI_Concert</b>가 먼저 있어야 합니다 — ②를 만드세요.','bad'); return; }
      st.zcConcert=true; setMsg('<b>ZC_Concert 활성화.</b> ZI_Concert를 투영해 소비용 뷰를 만들고 <code>_Perf</code>도 노출했습니다.','ok');
    }
    else if(id==='mdext'){
      ...
      st.preview=true; setMsg('<b>Data Preview.</b> 노출 필드는 달라도 행 수는 같습니다 — projection은 열만 고를 뿐 행을 바꾸지 않아요.','ok');
    }
    else if(id==='consume'){
