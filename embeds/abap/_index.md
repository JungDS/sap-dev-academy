...
| CH21-L05-S01 | CH21-L05 | interface-contract-board | 인터페이스 1(zif_printable)+구현 클래스 3·계약 보드·인터페이스 참조에 클래스 담기→같은 print() 다른 출력(다형성)·구현 누락→활성화 실패 | ✅ |
| CH21-L06-S01 | CH21-L06 | exception-flow-console | 좌석(정상/정원초과)×CATCH 순서(구체/cx_root 먼저)→TRY/RAISE/CATCH 실행 흐름 단계+cx_root 먼저 시 구체 handler 도달불가 경고 | ✅ |
...
| CH23-L03-S02 | CH23-L03 | association-path-simulator | (인터랙티브) 공연 선택→_Perf 경로 회차 따라가기·노출 토글(off→경로 못봄 warn)·concert_id 제거(ON source 사라짐 bad)·cardinality [1..1]↔회차 여러개 불일치 warn·JOIN으로 보기(CH13 INNER JOIN 비교) | ✅ |
| CH23-L04-S01 | CH23-L04 | annotation-effect-preview | Annotation 효과 미리보기 3패널(@EndUserText.label 토글→헤더 기술명↔업무라벨·@Semantics.amount.currencyCode 짝 currency_code=ok/자기참조 ticket_price=bad/없음=warn·@UI.lineItem ▲▼ 순서→position 재할당) | ✅ |
...
| CH23-L06-S01 | CH23-L06 | dcl-auth-comparator | DCL 권한 필터 결과 비교기·사용자(SEOUL/BUSAN/NO_AUTH)+PFCG 배지·@AccessControl #CHECK↔#NOT_REQUIRED 토글→같은 SELECT 다른 결과 행·#NOT_REQUIRED면 권한밖 venue 노출(unauth 행 빨강 tag·보안 실패 bad)·#CHECK 필터 ok | ✅ |
| CH23-L07-S01 | CH23-L07 | cds-builder-stepper | (capstone) 콘서트 CDS 빌더 6단계 순서 의존(①ZI_Perf②ZI_Concert+_Perf③ZC_Concert④DDLX⑤Data Preview⑥소비코드)·순서 어기면 target 없음 bad·산출물 4 active 다이어그램·preview ZI/ZC 행수 같음·도전과제 카드 | ✅ |
| CH24-L01-S01 | CH24-L01 | rap-layer-assembler | RAP 계층 조립 보드·6계층 토글(Root/Projection/BDEF/Behavior Pool/Service Def/Service Binding)→스택 채움·빠진 계층마다 막히는 사용자 행동(BDEF 없으면 CUD 불가 등)·다 채우면 RAP BO 완성 ok | ✅ |
| CH24-L02-S01 | CH24-L02 | root-entity-picker | RAP Root Entity 판정기(설계 판단)·후보 공연/회차/예매 선택→사용자 행동 모이나(예매만 belong)+key 후보 안정성(booking_id ok/status·customer bad)→define root view entity code·verdict 예매+안정key=ok/다른후보=warn/불안정key=bad | ✅ |
| CH24-L03-S01 | CH24-L03 | projection-contract-checker | Transactional Projection 계약 검사기·ZI_Booking→ZC_Booking·토글 contract(transactional_query)/projRoot/allowExt+노출필드 선택→DDL 라이브+payload chips·!projRoot=bad(root 불일치)·!contract=warn·정상 ok·감사필드 숨김=계약 제외 | ✅ |
...
| CH24-L05-S01 | CH24-L05 | behavior-pool-tracer | RAP Behavior Pool 호출 추적기·4단계 순서(저장 요청 keys 3행→READ ENTITIES 집합 read→정원 검사 B002/B003 초과→failed/reported 기록)·loop 안 EML 토글=anti-pattern warn·keys=internal table(집합 지향)·validation은 실패 알림만 | ✅ |
...
| interface-contract-board (신규) | 1 | 공통(_engine)+_autoheight ✅ | CH21-L05-S01 | ICB_CFG.classes 주도 · 계약 보드(intf 카드+클래스 카드 sel/miss)+cls 세그×impl 세그(구현됨/누락)→인터페이스 참조 코드+out(ok 다형성 다른 출력/bad 활성화 실패)·intf 보라(#7c3aed)·base=var(--surface) · 다크 |
| exception-flow-console (신규) | 1 | 공통(_engine)+_autoheight ✅ | CH21-L06-S01 | EFC_CFG 주도 · seats 세그(정상2/초과100, remaining 40)×order 세그(구체/cx_root 먼저)→코드(cx_root 먼저 시 구체 handler dead)+실행 흐름 li(ran/raise/caught/skip)+verdict(ok 정상/bad 실패처리/warn 순서문제)·3색 verdict base 중립rgba+.ok 명시(교훈3)·base=var(--surface) · 다크 |
...
| rap-layer-assembler (신규) | 1 | 공통(_engine)+_autoheight ✅ | CH24-L01-S01 | 데이터 내장 · 6계층 토글(root/proj/bdef/pool/svcdef/svcbind)→스택(present/missing·DB Table 고정)+verdict(누락 계층별 막히는 사용자 행동 ul·다 채우면 ok)·교훈3 base 중립rgba ok/warn·tog base=var(--surface) · 다크 |
...
| rap-booking-runtime (신규) | 1 | 공통(_engine)+_autoheight ✅ | CH24-L09-S01 | (capstone) 데이터 내장(CAP 10) · create(좌석2 ok/12 over)→determination status=N+validation(seats vs remaining)→save/reject·bookings 표+취소(action→status C·중복 disabled/no-op warn)·실행 타임라인(create→det→val pass/fail→save/reject node)·failed/reported 패널(reject 시)·잔여 라이브·msg ok/bad/warn(교훈3)·btn base=var(--surface) · 다크 |
