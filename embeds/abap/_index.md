...
| CH06-L07-S01 | CH06-L07 | kind-toggle-lab(신규) | 같은 코드, 종류만 STANDARD↔HASHED 토글 — 그릇 시각화(번호 선반 1·2·3번 vs "번호 없음" 사물함)·LOOP 순회 출력 **sy-tabix 1,2,3 vs 0,0,0** 대비(HASHED 순회 순서=시스템 관리 순서 명시)·**READ INDEX 2 시도 → STANDARD 성공/HASHED 구문 오류 카드**. LOOP는 `[선행 사용]` 표기(L08 정식 도입 전 — 훑는 길 관찰 용도만) | ✅ |
...
| CH06-L08-S01 | CH06-L08 | step-debugger(itab워치) | 2막 14스텝 — 1막 WHERE로 걸러 돌 때 반복 회차 vs sy-tabix 괴리(1·3·5) · 2막 **FROM 2 TO 5**(번호 구간, `LOOP AT … INTO … FROM 2 TO 5` 어순). *구 CH06-L04-S01에서 3막(ASSIGNING) 분리* | ✅ |
...
| CH06-L09-S01 | CH06-L09 | step-debugger(itab워치) | APPEND LINES OF 4스텝 — **본문 L09 변수 그대로(gt_seoul·gt_busan·gt_all)**: 서울 2행→부산 2행이 끝에 붙고(교체 아님) **두 지점 모두 신청한 홍길동 2회 입장**(중복 제거 다음 절 연결)·DESCRIBE로 원본 불변(복사) 확인. *구 CH06-L04-S04 rename+데이터 정합* | ✅ |
...
| kind-toggle-lab (신규) | 1 | 공통(_engine·KTL_CFG)+_autoheight ✅ | CH06-L07-S01 | 테이블 종류 토글 실험 — 종류 탭→선언 강조(공통 코드 고정)·그릇 시각화(번호/번호 없음)·[LOOP 돌려 보기] 순회 출력(sy-tabix 배지·0=빨강)·[READ INDEX 시도] ok/구문 오류 카드·토글 시 출력 리셋. config 주도(commonCode/kinds[decl·rows·loop·readTry]) · ≤640px 1단 · 토큰/color-mix 전용 |
