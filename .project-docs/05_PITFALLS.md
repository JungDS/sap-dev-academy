...
| **P3** | 미리보기 스크린샷이 자주 타임아웃되고, 깊은 URL 네비도 컨텍스트가 끊기기 쉽다. | **검증은 브라우저 eval의 DOM 측정**으로(정확도↑). 네비 후 별도 eval로 재측정. [07](07_BROWSER_TESTING.md). |
...
| **P10** | 코드 블록을 다크/블랙으로 만들지 말 것(R5의 반복 회귀 기록 — 새 규칙 아님). 합의 양식 = **[code-copy-block](../sample/structure/code-copy-block.html)**(네이비 헤더+줄번호+ABAP 토큰색+복사). | 본문 MD엔 ```` ```abap ```` 만 쓰고 **빌드가 `.abap-editor` 마크업으로 강제 변환** → 손으로 검은 블록이 나올 수 없다. 새 코드 표시도 이 양식 재사용([04 R5](04_CONVENTIONS.md)). |
