...
## ▶️ 다음 할 일 (우선순위)
1. **CH03 보강** — 동일 프로토콜(챕터 전 레슨 정독 → 델타 식별 → `C:\ABAP_DOCU_HTML` 사실검증 → R3/R6/R15 통과분만 반영 → 챕터 단위 검수·1커밋) + **R2 갭 위젯 제작 포함**(A안 확정 — 기존 엔진 재사용 우선·신규는 [AUTHORING](../embeds/_engine/AUTHORING.md) 계약·"변수/개념의 모양을 그린다" 지향). ⚠️ codex_0629_v3는 CH01～02만 존재 — CH03부터는 codex_0625(메타 진단)·antigravity_0629_v1(선별)만 참고. 잔여 R2 목록 = `CONTENT_DEPTH_AUDIT`(재생성물).
