...
## ▶️ 다음 할 일 (우선순위)
1. **매트릭스 캠페인** — CH01 파일럿 완료 → **① 전량 진행 go/no-go(사용자)** → go 시 CH02+ 배치(하루 3～5챕터) + 커리큘럼 축 AG07/08 1회([15 §7](15_AUDIT_MATRIX.md)). ⚠️ agy 주간 쿼터: 파일럿에서 실행 14회(재시도 포함 21시도) 소진 — 전량 시 챕터당 7회 페이싱 유의. ② CH01 수정 착수 여부(별도 지시 — 백로그 = [verdict/CH01.md](../.archive/2026-08-03-matrix-audit/verdict/CH01.md)). 별건 대기 = CH37-L04-S01 문구(4장→5장, verdict와 별개 기확인)·`.gemini/rules` 처리(커밋됨 — gitignore 전환 여부).
