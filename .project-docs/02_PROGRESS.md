...
## 🎯 현재 초점
**CH31 Editable Grid ALV 보강 완료 → 다음 CH32(Enhancement/BAdI/User Exit) 순차 보강**(브랜치 `content/ch31-editable`, main=`c6838fb`). 원칙: content 기준, `reference/codex_0629_v3` 참고(QA+REWRITE 동시 대조) + 델타 사실검증([14 §5·§6](14_REFERENCE_CORPUS.md)) + 게이팅(R10/R15). **챕터 실행 = [10_REBUILD_EXECUTION](10_REBUILD_EXECUTION.md)**.

## ▶️ 다음 할 일 (우선순위)
1. **CH32(Enhancement/BAdI/User Exit)+ 순차 보강**. ⚠️ codex 파일 = 우리 번호 일치(`NEWCH32_OLDCH29_*`) · codex 본문 내 구번호 잔재 주의(임베드 지시자 `:::`+구ID 복붙 금지).
   ⚠️ **`embeds/abap/_index.md` 등재 갭**: CH32～39 위젯(43건)이 A/B 미등재 — CH31처럼 **각 챕터 패스에서 자기 몫 등재**(CH31은 완료).
   ℹ️ **CH31 미검증 영역(코퍼스 부재·본문 유지, ZABAP_JHY 실코드와 표기 일치 확인)**: CL_GUI_ALV_GRID 이벤트(data_changed/_finished)·er_data_changed/e_modified·register_edit_event/mc_evt_*·CL_ALV_CHANGED_DATA_PROTOCOL(add_protocol_entry/display_protocol)·check_changed_data(e_valid)·set_ready_for_input·mc_style_*·LVC_* 구조 = SE24/SE11 영역. 언어 레벨(FOR EVENT·SET HANDLER·MODIFY dbtab subrc/dbcnt·COMMIT/ROLLBACK·CONV+cx_sy_conversion_no_number·정렬테이블 APPEND 조건·SQL wa deep 금지)은 두 루트 교차 확정. 실기 확인 잔여: mt_good_cells(본문)↔mt_mod_cells(실코드) 관례차 · mc_evt_modified 발화 시점 · 숫자 컬럼 문자입력의 ALV 선차단.
   ℹ️ **codex 오류 패턴(계승)**: NEWCH31 L06이 deep 컴포넌트(cellstyles) 포함 행을 그대로 `MODIFY dbtab FROM TABLE` — **SQL wa deep 금지 위반(컴파일 불가)**. 표시용 deep 구조는 flat으로 `CORRESPONDING` 변환 후 DB 반영(우리 본문은 flat `gt_booking`이라 무관). sy-dbcnt는 COMMIT 전에 보관.
