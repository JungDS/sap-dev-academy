...
## ▶️ 다음 할 일 (우선순위)
1. **매트릭스 캠페인 진행** — 설계 확정 완료(§1 대기 0) + **하네스 구현 완료(2026-08-03, `tools/audit-matrix/` — 조립·실행기·검증기·스코어카드, CH01 조립 및 스모크 통과)** → 다음 = **CH01 파일럿 28+1벌**([15 §8](15_AUDIT_MATRIX.md) 합격 기준) → go/no-go 보고 → 전량(하루 3～5챕터, [15 §7](15_AUDIT_MATRIX.md)). ⚠️ agy 주간 쿼터 페이싱 유의. 별건 대기 = CH37-L04-S01 문구(4장→5장)·`.gemini/rules` 처리.
