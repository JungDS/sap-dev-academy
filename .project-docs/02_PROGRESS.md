...
## 🎯 현재 초점
**CH20 Advanced SQL 신규 장 완료 → 다음 CH21 OO 보강**(브랜치 `content/ch20-advanced-sql`). 리넘버·CH01～19 **main 병합**(PR #14·#15). **CH20 완료**: 본문 7레슨 + 위젯 7종(신규 엔진 5 + 재사용 2, 라이트/다크·콘솔 0 검증) + glossary 4 + CH19 "다음→" 재배선(빌드 37장/250페이지·parity 0). 원칙: content 기준, `reference/codex_0629_v3` 참고 + 델타 사실검증([14 §5](14_REFERENCE_CORPUS.md)) + 게이팅(R10/R15). **챕터 실행 = [10_REBUILD_EXECUTION](10_REBUILD_EXECUTION.md)**. ⚠️ 리넘버 후 **우리 CHnn = codex NEWCHnn 일치**.

## ▶️ 다음 할 일 (우선순위)
1. **CH21 OO 보강**(구 CH20). 조사 완료(홀드, 세부=이 세션 워크플로): **L06 T100 예외 텍스트 + `THROW` expression 부재(high)** · L10 raise T100 미연결(med) · L01~05 low(L02 `mv_capacity`/`mv_booked`→관통예제 이름 정렬·L01 `CREATE PUBLIC` 설명·L03 없는 공연ID 주의). codex `NEWCH21_OLDCH20`.
   ✅ **CH20 Advanced SQL 완료(2026-07-14)**: 본문 7레슨 + 위젯 7종(신규 엔진 cte-step-viewer·exists-vs-join·sql-set-board·window-partition-sim·concert-adv-sql-lab 5 + compare-matrix·decision-tree 재사용 2)·사실검증 반증 0·glossary +4·CH19 재배선. 세부=git.
2. **CH22+ 순차 보강**(구 CH21+). ⚠️ 리넘버 후 **codex 파일 = 우리 번호 일치**(CH21=`NEWCH21`…CH39=`NEWCH39`). 신규 CH28 Dynamic ABAP(8L)·CH29 Regex(8L)는 진도가 닿을 때 예약 슬롯에 집필(추가 리넘버 0).
