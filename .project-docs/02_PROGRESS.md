...
## 🎯 현재 초점
**승인 플랜 3작업 중 ①②완료 → ③ W0+W1 감사만 남음(플랜 = `~/.claude/plans/track-a-drifting-donut.md`, W0+W1 승인 2026-08-02).** ① 관통예제 배지(PR #36 머지) · ② CH37 OTF 재구성(브랜치 `content/ch37-otf` — Adobe/ADS 전량 제거, L02 = getotf/no_dialog/output_options/user_settings 3종 세트→`CONVERT_OTF`, L04 = `bin_file`(xstring) 재방문, 제목 "PDF 바이트와 다운로드"로 교체). 관통 확정 범위 = CH01～31·33·36·37·39(독립 모듈 = 32·34·35·38, [09 §A](09_CURRICULUM_LEDGER.md)). 원칙: content 기준 + 게이팅(R10/R15) · 시뮬레이션 운영형 = [08 §10-12](08_LESSON_SHELL_SPEC.md).

## ▶️ 다음 할 일 (우선순위)
1. **작업 3 — W0+W1 감사**(순서·게이팅 도구 신작 `tools/audit-gating.mjs` → 축1·축3(Track 구성 백지 검토 — 순서 유지 전제, 복수 안+권고 리포트만)·축6 → 웨이브 보고 후 W2 결정. 계획 상세 = 메모리 `full-curriculum-audit-plan.md` + 승인 플랜).
