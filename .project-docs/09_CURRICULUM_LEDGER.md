...
| 22 | SALV/Grid ALV 표시 제어 심화 | 8 | Cell Color/Style·Stable Refresh·SALV-OOP 이벤트 *체험* |
...
| 28 | Dynamic ABAP: Field Symbol 심화·Generic 🆕 | 8 | Field Symbol 심화·`ASSIGN`·RTTS/RTTI·동적 SQL/호출 |
...
| 33 | 인터페이스 실무: BAPI/RFC/BDC/File | 5 | BAPI·RFC·BDC·OPEN DATASET |
| 34 | IDoc / ALE / Gateway | 5 | IDoc·ALE·OData/Gateway |
| 35 | 성능 분석과 튜닝 | 5 | SQL Trace·런타임 분석·SELECT 최적화 |
...
> 🆕 **CH20 Advanced SQL · CH28 Dynamic ABAP · CH29 Regex = 신규 장** — 2026-07-14 리넘버로 슬롯 확보(codex `NEWCH20/28/29_OLDCH99`). **본문 미집필 = 폴더 미생성**(로드맵 번호 공백, 셸은 정상). 집필 시 예약 슬롯에 삽입(추가 리넘버 0).
...
→ "메모리 itab을 **통째로 INSERT**·감사필드 자동 stamp" = **CH25(DML)로 foreshadow**(L1). CH09부터 업무 데이터.
...
- 이후 등장할 때마다 값 변화를 그 자리에서: `sy-tabix`(CH06 LOOP)·`sy-subrc`(CH06 READ / CH08 SELECT — 핵심)·`sy-datum` 기본값(CH15 INITIALIZATION)·감사 stamp(CH25). 한곳 몰아넣기 금지.
...
- **DML·감사필드·표준테이블 규율 = CH25(Track-2)로 이관.** Track-1 CH08은 **읽기 전용**(INSERT/UPDATE/DELETE 미포함). 표준 테이블(SCARR 등)은 읽기 전용·직접 DML 금지(BAPI 경유) 원칙도 CH25. Lock(ENQUEUE) = CH26.

## E. Track-2 다운스트림 관계 (CH25～39) — Track-1과의 잇기
| CH | 챕터 | Track-1과의 관계 |
|---|---|---|
| 25 | 실무 데이터 변경·트랜잭션(DML) | CH08 읽기 → 여기 쓰기 (감사 stamp·통째 INSERT·표준테이블 규율) |
| 26 | Lock·동시성 | CH25와 짝 |
| 27 | OO 고급 패턴 | CH21 기본 → 심화 |
| 28 | Dynamic ABAP 🆕 | CH06 Field Symbol 기초 → RTTS/동적 |
| 29 | Advanced String/Regex 🆕 | CH04 문자열 → 정규식 심화 |
| 30 | ALV 고급 Event | CH22 OOP 체험 → 본격 이벤트 |
| 31 | Editable Grid ALV | CH17 표시 → 입력 |
| 32～39 | Enhancement·인터페이스·IDoc·성능·AMDP·Forms·운영·Capstone | 실무 응용 |
