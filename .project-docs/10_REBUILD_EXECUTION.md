...
2. **codex 파일 매핑**: 2026-07-14 리넘버 후 **우리 CHnn = codex `NEWCHnn_*` 1:1 일치**(전 챕터).
...
`사실검증(두 루트[+코퍼스 얇으면 zabap_jhy]) → 게이팅(R6/R15) → REWRITE 델타 → QA 소급 대조(§2/§5/§8) → 톤·컨벤션 sweep → 위젯 리뷰(본문↔위젯 정합·다크·수평스크롤·콘솔) → **학습자 시뮬레이션**(페르소나 상이 에이전트 2～3 — 이해도·can-do·납득 레슨별 점수화, 낮은 레슨 재보강 후 재평가·[08 §10-9~12](08_LESSON_SHELL_SPEC.md))` → 판정 → 적용 → 검증 → 커밋.
...
- **오류만이 아니라 결손도 의심** — codex가 안 다룬 것(생성 절차·호출 코드·도구의 존재 이유·존재증명 예제)을 그대로 계승하지 말 것(CH36 선례: 구조 해부만 있고 만드는 법·왜가 없었음).
...
> ⚠️ 전제: CH01～36 본문은 **이미 존재**(CH04 삽입·리넘버 반영 완료) → greenfield 아님·**리넘버 단계 없음**. 골든 5종은 *품질 참조·재사용 씨앗*이지 맞출 틀이 아니다.
...
- **A2 병렬 드래프팅 CH09～24 + CH25～39 (에이전트 ✅·챕터 단위)** — 챕터마다 서브에이전트 1개(서로 다른 폴더 = 충돌 0). 각 에이전트 brief 필수:
