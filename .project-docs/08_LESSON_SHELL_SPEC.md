...
| 3 | ⓓ DDIC 객체 생성 | **CH03-L01** Domain(얕게) | 도메인 만들기 체험(저장→검사→활성화) + SE11 미니페이지 | DDIC 생성 유형 대표(CH03·04·06·08·13·21 재사용)·T코드 라벨 시연 |
...
> **진행:** ✅ #1 CH14-L01(흐름)·✅ #2 CH01-L04(코드) 생성·검증 완료. 남은 3 = #3 DDIC·#4 SQL·#5 ALV. #2의 학습수단은 당초 후보(step-debugger)보다 WRITE에 직접적인 **출력 시뮬레이터를 신규 제작**(§9.1 "샘플에 없으면 신규 제작" 규칙 적용).
