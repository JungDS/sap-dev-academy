...
|---|---|---|---|---|
| 1 | ⓘ 이벤트·실행 흐름 | **CH14-L01** ABAP Report Event 전체 흐름 | `visuals/mermaid-flowchart.html`(이벤트 라이프사이클) + process-flow | "위→아래 실행이 아니다" 강한 동기 훅·흐름 시각화 최적, 다이어그램 유형 대표 |
| 2 | ⓑ 코드 작성·실행 | **CH01-L04** WRITE로 문자열 출력 | `code-learning/step-debugger.html` + 코드복사 + 출력 시뮬 | 가장 흔한 레슨 유형(모든 구문 레슨의 원형)·진입 가치 최고 |
