...
- [ ] (P1) `reference/tcodes.json` 스키마 + 초기 T코드(SE38·SE11·SE16N·SE80…) 작성, 런타임 직접 fetch(빌드 안 함). **(다음)**
...
- [ ] **텍스트 위주 지양**: 시각/상호작용은 *값을 하는 곳에* 충분히(텍스트만으로 충분한 섹션은 허용) — 레슨 전체 텍스트-only만 금지(§9.4 진단).
...
3. **스키마·게이트 잠금** — 04 R10(`tcode`/`tcodeBadge`/`goals`)·본 문서 갱신, glossary 패리티 점검, §9.4 지표 정의.
...
### 9.4 "텍스트 위주" 검증 지표
- preview_eval로 각 `.prose`(또는 섹션) 내 **시각/상호작용 노드**(`.embed`·`.term`·`table`·`blockquote/callout`·`img/svg`·`details`) 집계 → **요소 0인 순수 텍스트 섹션 = *리뷰 플래그*(게이트 아님 — 텍스트만으로 충분하면 통과, 장식용 강제 금지).** 단 **레슨 전체가 텍스트-only면 보강 필수.** 전체 텍스트:상호작용 비율 리포트.
...
5. **텍스트 위주 지양**: 시각/상호작용은 필요·적합한 곳에(텍스트만으로 충분하면 그대로 OK), 레슨 전체 텍스트-only만 금지(§9.4 진단 플래그).
