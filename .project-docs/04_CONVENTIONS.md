...
- **다른 챕터 참조 = 링크(내부ID 비노출)** — 본문에 `CHxx`(내부 ID)를 그대로 쓰지 말고 `[Chapter NN · 제목](CHnn-L01.html)`(그 챕터 첫 레슨 페이지)로 — 사람이 읽고 클릭 이동. 불편 체인 `→ CHxx`도 동일. (레슨 참조는 `[CHxx-Lnn 제목](CHxx-Lnn.html)`.)
