...
- 코드는 ```` ```abap … ``` ````. **빌드가 이 fenced 블록을 합의된 [code-copy-block 양식](../sample/structure/code-copy-block.html)(네이비 헤더 + 줄번호 + ABAP 토큰색 + 복사 버튼)으로 자동 변환**한다. → 다크/블랙 코드 블록은 만들지 않는다([05 P10](05_PITFALLS.md)). 본문 MD엔 그냥 fenced 코드만 쓰면 된다(직접 HTML 마크업 금지).
...
## R8. 네이밍
- 챕터 폴더 = 챕터 ID(`CH01/`). 레슨 **소스** 파일 = `<레슨ID>[-슬러그].md`(슬러그는 선택, 사람이 읽기 편하라고).
- 레슨 **생성물(HTML)** 파일명은 **슬러그 무관하게 레슨 ID로 통일** = `docs/abap/pages/<레슨ID>.html`(예: `CH01-L04.html`). 빌드가 강제(슬러그 잔재는 빌드 시 pages 폴더를 비워 제거). 내부 링크(curriculum.json·prev/next·rail)도 이 경로를 쓴다.
