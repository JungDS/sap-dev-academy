...
## R1. 타임스탬프
- 손작성하는 `.md`(content·.project-docs)는 최상단/헤더에 `YYYY-MM-DD KST` 갱신.
...
## R3. 본문(MD) 규칙
- 섹션은 `##`(필요시 `###`) — 네비 "이 문서" 탭이 자동 목차로 만든다.
- 코드는 ```` ```abap … ``` ````.
- 핵심용어: `[[WRITE]]` 또는 `[[Internal Table|내부 테이블]]` → 셸이 `term` 버튼으로 변환, 클릭 시 glossary 팝업.
- 콜아웃은 `>` 블록인용. 챕터/레슨 끝에 다음 불편을 `→ CHxx`로 명시(불편 체인).

## R4. 빌드 락 (생성물 직접수정 금지)
- **HTML/JSON은 생성물** — 내용은 `content/**.md`에서만. `npm run build:abap`로 재생성([01 R1](01_AI_SYNC.md)).
