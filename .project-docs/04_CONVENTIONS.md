...
- `_chapter.md`: `id`(=폴더명)·`track`(리터럴 **`TRACK-01`/`TRACK-02`** — `_tracks.md`와 일치, 빌드 join 키)·`order`·`title`·`intro`·`keywords`·`difficulty` · (선택) `thread: false` = 관통예제(콘서트 예매) 밖 **독립 모듈** 선언(키 없음=관통이 기본값; 로드맵 칩·레슨 태그로 표시, 소속 목록은 [09 §A](09_CURRICULUM_LEDGER.md)).
