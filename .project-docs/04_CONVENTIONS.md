...
> 📖 파일을 쓰기/고치기 직전. 중요도 순(필수→높음→중), 안정 ID `Rn`(추가만·번호 재사용 금지 — 번호 비순차는 안정 ID 유지).
...
- 친근한 톤 · **이모지/SVG 둘 다 상황껏**(한 방식 강제 금지).
...
- **ABAP 고유 용어는 영어 원문**(직역 금지): Internal Table·Data Element… 풀이는 괄호로.
- 흐름: 왜 필요한가 → 무엇인가 → 어떻게 쓰나 → 실수/주의 → 정리. 압축 설명 금지.
- **줄표(—) 절제** — 허용 = 제목·캡션·리스트/번호 라벨(`- **라벨** — `·`1. **라벨** — `)·코드 주석·콜아웃 라벨의 "주제 — 부제" 구분. **본문 문장 안 부연은 마침표 분리/괄호/콜론으로**(줄표 과용 = 호흡 길어져 입문자 가독성↓, 사용자 확정 2026-07-03).
- 시각화·체험 동반(길면 `details` 접기). 참고 구현: `sample/structure/beginner-lesson-template.html`.
...
- 체험 임베드: `::embed CHnn-Lnn-Snn | <제목> | <높이>::` → `embeds/abap/CHnn-Lnn-Snn.html`(공통 엔진 `embeds/_engine/` + 레슨 위젯)을 iframe으로. `sample/`은 참고 카탈로그([06](06_SAMPLE_LIBRARY.md)) — 직접 참조 금지.
- 콜아웃은 `>` 블록인용. 레슨 끝 불편 체인 `→ CHxx`의 성립 조건·대안 = R15(억지 불편 금지).
...
**R6 · classic-first 경계** — modern 문법·SQL은 정해진 챕터부터만 — **R15 게이팅의 특례(일반보다 강함)**, 도입(CH18/19) 후 자유 사용.
...
**R15 · 개념 노출 게이팅 + 선수지식 잠금 ★** (R6와 짝) — 레슨은 **그 시점까지 배운 것만으로** 이해·실습 가능해야 한다. 후속 개념의 정의·구조·문법·코드를 미리 당겨 설명하지 않는다(대표 실수 = [05 P11](05_PITFALLS.md)). 같은 개념도 노출을 4단계로 관리:
...
- **R6 나선과의 화해**: 나선 재방문은 "같은 개념 중복 정의"가 아니라 **범위가 다른 별개 개념**으로 본다(예: Local 구조체 ≠ DDIC 구조체 → 각자 L3 도입 지점). 경계는 R10 front-matter(`introduces`/`prereq`/`foreshadow`/`advanceUse`)로 선언.
...
**R9 · 이름 풀 ★** — 예제 사람 이름은 **아래 풀에서만**(즉석·어색한 작명 금지), **1번은 항상 정훈영**. 같은 캐릭터는 파일이 달라도 동일 이름.
- 주인공: **정훈영**
...
- `_chapter.md`: `id`(=폴더명)·`track`(리터럴 **`TRACK-01`/`TRACK-02`/`TRACK-03`** — `_tracks.md`와 일치, 빌드 join 키. 3분할 = CH01～17/18～24/25～39, 사용자 확정 2026-08-02)·`order`·`title`·`intro`·`keywords`·`difficulty` · (선택) `thread: false` = 관통예제(콘서트 예매) 밖 **독립 모듈** 선언(키 없음=관통이 기본값; 로드맵 칩·레슨 태그로 표시, 소속 목록은 [09 §A](09_CURRICULUM_LEDGER.md)).
- 레슨 `.md`: `id`·`title`·`direction`·`keywords`·`order` · (선택) `tcode`·`tcodeBadge`·`goals`.
- 레슨 `.md` **학습 경계(R15)**: `introduces`(이번에 L3 정식 도입할 개념)·`prereq`(전제 개념) = **리빌딩 시 필수**(게이팅 검증 근거) · `foreshadow`(L1 예고 허용)·`advanceUse`(L2 선행 사용 허용)·`prevRel`(이전 레슨과의 관계: `pain-solution`/`parallel`/`deepening`/**`next-step`**(불편 서사·심화가 아닌 같은 흐름의 다음 단계 — 사용자 확정 2026-08-18)) = 선택. 빌드는 무시(메타 전용·정적 점검 근거).

**R11 · 네이밍** — 챕터 폴더 = 챕터 ID(`CH01/`). 레슨 소스 = `<레슨ID>[-슬러그].md`. **생성물 HTML = `docs/abap/pages/<레슨ID>.html`**(슬러그 무관, 빌드 강제). 샘플 = `<카테고리>/<기능-케밥>.html`.
- **ABAP 변수 접두어(스코프) ★** — Subroutine/Method(로컬 스코프) 도입 **전(=CH01～09)** 에는 모든 선언이 사실상 **전역**이다 → **`gv_`(스칼라)·`gs_`(Structure)·`gt_`(Internal Table)** 로 시작. 로컬 스코프(METHOD/FORM 안, **CH10 모듈화 기초↑**)에서 선언한 것만 `lv_`/`ls_`/`lt_`. 즉 g=global·l=local. **`it_`는 Header Line을 가진 Internal Table에만** (헤더라인 없는 일반 내부테이블은 `gt_`/`lt_`). **Module Pool 화면필드에 묶인 전역 DATA도 `gv_`**(화면 요소명=변수명이라 Screen Painter 요소도 `GV_…`, `p_` 예외 없음 — 사용자 확정 2026-07-08). 단 **OK field는 프레임워크 관례명 `ok_code`/`save_ok` 유지**(스코프 접두어 미적용, `sy-ucomm`과 동류). **Range Table 접두어 = `gr_`(전역)/`lr_`(로컬)** — `sign`·`option`·`low`·`high`만 있는 특수 테이블이라 일반 Internal Table(`gt_`)과 구분하며, 실무도 `GR_`/`R_` 별도 명칭이 일반적(사용자 확정 2026-08-10). **객체 참조 접두어 = `go_`(전역)/`lo_`(로컬)** — `TYPE REF TO`로 선언한 참조변수에 적용(사용자 확정 2026-08-15). **클래스 속성 접두어(CH21+) = 인스턴스 `m*` · 정적(`CLASS-DATA`) `g*`** — 정적 속성은 클래스 안의 전역 변수나 마찬가지라 전역 접두어를 그대로 쓰며, 뒷글자는 값 종류(v/s/t/o)를 따른다(`mv_`/`gv_`·`ms_`/`gs_`·`mt_`/`gt_`·`mo_`/`go_`, 사용자 확정 2026-08-23). **파라미터 접두어 = 방향 글자 + 값 종류** — IMPORTING `i*`·EXPORTING `e*`·CHANGING `c*`·TABLES `t*`(예: `iv_`/`ev_`/`cv_`/`it_`… — 이벤트 `EVENTS … EXPORTING` 파라미터도 `ev_`, handler는 같은 이름으로 받음, 사용자 확정 2026-08-23).
- **본문 예제 조각의 접두어 해석 ★** — FORM/METHOD 껍데기 없이 제시되는 레슨 예제 코드 조각은, **CH10(모듈화) 이후 챕터에서는 로컬 문맥(메서드/FORM 안에 들어갈 코드의 발췌)을 전제로 `l` 접두어 허용**(사용자 확정 2026-08-18). 전역 위치로 명시·서술된 코드(화면 필드·Module Pool 전역 DATA·"전체 골격" 등)는 여전히 `g`. 기존에 `g`로 통일한 챕터(CH17·CH20 등)도 합법 — 챕터 내 일관성만 유지.
- **표현식 내부 변수 = 맨 이름 허용** — `FOR`·`REDUCE`(`INIT`)·`LET` 등 표현식 안에서만 사는 보조 변수(`i`·`line`·`sum` 등)는 스코프 접두어 없이 맨 이름을 쓴다(SAP 공식 예제·Clean ABAP 관례, 사용자 확정 2026-08-18).
- **로컬 TYPES 이름 접두어** — 스칼라 `ty_` · **Structure 타입 `ts_`** · **Table Type `tt_`** (BC400 표준 교재 관례, 사용자 확정 2026-07-03).
- **PARAMETERS 접두어 = `pa_`** (표준 교재 관례 — 실무 축약 `p_`는 참고 언급만, 사용자 확정 2026-07-03). **SELECT-OPTIONS 접두어 = `so_`** (표준 관례 — 실무 축약 `s_`는 참고만, 사용자 확정 2026-07-08). 이름은 8자 이내(`so_`+최대 5자).
...
**R14 · `../sapui5`(구 repo) 읽기 전용** — 이 저장소 옆 폴더의 **상세 커리큘럼 참고 원본**. 절대 수정 금지. **NotebookLM 질의는 sapui5 챕터ID로** 해야 답이 온다.

**R16 · `02_PROGRESS` = prune-only 스냅샷 ★** — `02_PROGRESS.md`는 **현재 상태·다음 할 일만** 담는다(누적 로그 아님). 완료/과거 항목·세션 서사는 **즉시 제거** — 끝난 일의 정본 = **git 이력 + `.archive/` 원장 + 라이브 인덱스**. ① 코드·git·감사로 **파생 가능한 현황은 복창 금지, 포인터만**(콘텐츠 깊이=`CONTENT_DEPTH_AUDIT`·embed=`embeds/abap/_index.md`·구조=`09`·완료이력=git/`.archive`) — 복창하는 순간 stale의 씨앗. ② **수정하면 같은 커밋에 포함**(git이 history). ③ 세션 종료 시 갱신([01](01_AI_SYNC.md) 종료 체크리스트).
