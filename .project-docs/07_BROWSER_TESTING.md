...
## 검증 순서
1. 브라우저 eval로 이동(`location.href='…'`) → **다음 eval은 새로** 보낸다(네비 후 컨텍스트 끊김 — 근거 [05 P3](05_PITFALLS.md)).
2. 브라우저 eval로 **DOM 측정**(요소 존재·텍스트·`getComputedStyle`·`getBoundingClientRect`·클래스 토글·클릭 후 상태) — **1순위**.
3. 콘솔 로그(error 필터) → **콘솔 에러 0** 확인.
4. (보조) 스크린샷 — 이 환경선 자주 타임아웃([05 P3](05_PITFALLS.md)). 실패해도 2(DOM 측정)로 결론 가능.
