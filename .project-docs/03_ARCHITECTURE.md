...
> ⚡ **TL;DR:**
> - **소스 = `content/abap/**.md`** / **생성물 = `docs/abap/**`(JSON·HTML)** / **참조 데이터 = `reference/`(glossary·tcodes — 손작성·런타임 직접 fetch)**. 생성물은 손대지 않는다([04 R1](04_CONVENTIONS.md)).
...
| 경로 | 역할 | 소스/생성물 |
|------|------|------------|
| `content/abap/**.md` | 레슨·챕터 **소스**(손작성·검수 대상) | 소스 |
| `reference/` | **참조 데이터**(glossary·tcodes) — 손작성·런타임 직접 fetch(빌드 안 함·cross-cutting) | 소스(참조) |
...
content/abap/**.md   (소스)
        ...
        ▼
docs/abap/curriculum.json · lessons/CHxx.json · pages/*.html  (glossary·tcodes는 reference/에서 직접 fetch)
        │  런타임
        ▼
...
- **참조 데이터(glossary·tcodes)는 `content/abap`이 아니라 루트 `reference/`** — 손작성, 런타임(shell.js)이 `siteRoot+reference/`로 직접 fetch(빌드 안 함). glossary는 도메인 공용(cross-cutting).
